...
import pandas as pd
import requests
from tests.test_stores import store_service

st.title('Stores')

col1, col2, col3 = st.columns(3)
col1.metric("Total stores", "2345", "Store Dashboard")
...
if st.button('Create Store'):
    ...
    if submit_button:
        ...
        create_store_url = "http://localhost:8501/test/test_stores"  # Replace with the actual URL
        ...
        if response.status_code == 200:
            ...
            st.error('Failed to create the store. Please check the server response.')
